...
def load_dataset(data_path: str) -> pd.DataFrame:
    ...
    df["Kf_Km_group"] = (
        df["Kf"].map(lambda x: f"{x:g}") + "_" + df["Km"].map(lambda x: f"{x:g}")
    )
...
def evaluate_group_kfold_by_col(df: pd.DataFrame, out_dir: Path, group_col: str) -> dict:
    X = df[FEATURE_COLS].values.astype(np.float32)
    y = df[TARGET_COLS].values.astype(np.float32)
    ...
    df_pred.to_csv(
        out_dir / f"predictions_group_kfold_by_{safe_group_name}.csv",
        index=False,
    )

    metrics_overall = calc_metrics(
        df_pred[TARGET_COLS].values,
        df_pred[[f"{t}_pred" for t in TARGET_COLS]].values,
        TARGET_COLS,
    )
    ...
    save_json(out_dir / f"metrics_group_kfold_by_{safe_group_name}.json", report)
...
def main() -> None:
    ...
    group_report_r = evaluate_group_kfold_by_col(df, out_dir, "r")
    group_report_kf_km = evaluate_group_kfold_by_col(df, out_dir, "Kf_Km_group")

    make_summary_md(out_dir, in_sample_metrics, random_holdout_metrics, group_report_r)
